...
import { CalendarClock, CheckCircle2, ClipboardList, Copy, ExternalLink, FileText, IdCard, Laptop, Link2, Loader2, MapPin, MessageCircle, MessagesSquare, PhoneCall, QrCode, Receipt, Send, ShieldAlert, TriangleAlert, Volume2, VolumeX, Smartphone, Tablet, Trash2, Download, Truck, Users, X, XCircle } from "lucide-react";
...
export default function Admin() {
  ...
  const remove = trpc.orders.delete.useMutation({ onSuccess: () => { toast.success("حُذف الطلب وملفاته"); refetch(); }, onError: error => toast.error(error.message || "تعذر حذف الطلب") });
  const deleteOrder = (order: { id: number; orderNumber: string; customerName: string }) => {
    if (!window.confirm(`حذف الطلب ${order.orderNumber} للعميل ${order.customerName} نهائياً مع وثيقة الهوية والإيصال؟ لا يمكن التراجع.`)) return;
    remove.mutate({ id: order.id });
  };
  const exportCsv = () => {
    const cols: [string, (o: (typeof orders)[number]) => unknown][] = [
      ["رقم الطلب", o => o.orderNumber], ["التاريخ", o => new Date(o.createdAt).toLocaleString("ar-SY")], ["الحالة", o => statusLabels[o.status]],
      ["العميل", o => o.customerName], ["الهاتف", o => o.phone], ["هاتف بديل", o => o.alternatePhone ?? ""], ["العمر", o => o.age], ["طبيعة العمل", o => o.jobNature], ["التزامات سابقة", o => o.hasExistingInstallments === "yes" ? "نعم" : "لا"],
      ["الجهاز", o => o.productTitle], ["الهدية", o => o.gift], ["السعر $", o => o.priceUsd], ["الدفعة الأولى $", o => o.downPaymentUsd], ["الخصم %", o => o.discountPercent], ["الأشهر", o => o.months], ["القسط الشهري $", o => o.monthlyInstallmentUsd], ["القسط الشهري ل.س", o => o.monthlyInstallmentSyp],
      ["المحافظة", o => o.province], ["المنطقة", o => o.area], ["معلم قريب", o => o.landmark ?? ""], ["اسم المستلم", o => o.recipientName],
      ["نوع الهوية", o => o.identityDocumentType], ["حالة الإيصال", o => o.paymentProofStatus], ["تاريخ مراجعة الإيصال", o => o.paymentProofReviewedAt ? new Date(o.paymentProofReviewedAt).toLocaleString("ar-SY") : ""],
    ];
    const cell = (v: unknown) => `"${String(v ?? "").replace(/"/g, '""')}"`;
    const csv = [cols.map(c => cell(c[0])).join(","), ...orders.map(o => cols.map(c => cell(c[1](o))).join(","))].join("\r\n");
    const blob = new Blob(["\uFEFF" + csv], { type: "text/csv;charset=utf-8" });
    const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = `horizonline-orders-${new Date().toISOString().slice(0, 10)}.csv`; a.click(); URL.revokeObjectURL(a.href);
  };
