...
function ProductDetails({ product, onBack, onCheckout, onCart }: { product: CatalogProduct; onBack: () => void; onCheckout: (product: CatalogProduct) => void; onCart: (product: CatalogProduct) => void }) {
  ...
  const colorClass = (name: string) => name.includes("برتقالي") || name.includes("Orange") ? "bg-[#de6e28]" : name.includes("فضي") || name.includes("Silver") || name.includes("Gray") ? "bg-[#d9dee2] ring-1 ring-slate-300" : name.includes("كحلي") || name.includes("Blue") || name.includes("Navy") ? "bg-[#1c324b]" : name.includes("ذهبي") || name.includes("Gold") ? "bg-[#d2a94b]" : "bg-[#80e1d5]";
  ...
  return <div className="min-h-screen bg-[#f7fafb]" dir="rtl">
    ...
    <main className="mx-auto grid max-w-6xl gap-8 px-5 py-10 lg:grid-cols-2 lg:items-center">
      ...
      <div className="order-2 lg:order-1"><span className="rounded-full bg-[#eaf8f4] px-3 py-1.5 text-xs font-extrabold text-[#17845e]">{brandOf(product)}</span><h1 className="mt-5 text-4xl font-extrabold tracking-tight text-[#071d32]">{product.title}</h1><p className="mt-3 text-sm leading-7 text-slate-500">{product.description || "هاتف ذكي متاح مع خطة تقسيط مرنة."}</p>
        ...
        <div className="mt-5 rounded-2xl bg-white p-5 shadow-sm"><p className="font-mono text-3xl font-bold text-[#071d32]">${money(price)}</p><p className="mt-1 text-sm font-bold text-slate-500">{lira(price)} ل.س تقريباً</p><p className="mt-4 flex items-center gap-2 text-xs font-bold text-[#17845e]"><Check className="h-4 w-4" />من ${money(Math.max(0, (price - 100) / 48))} شهرياً عند خطة 48 شهراً</p></div>
...
function ProductCard({ product, details, add }: { product: CatalogProduct; details: () => void; add: () => void }) { const price = priceOf(product); return <article className="product-card"><div className="product-image-wrap relative"><span className="absolute right-4 top-4 z-10 rounded-full bg-white/90 px-3 py-1.5 text-xs font-extrabold text-[#25887e]">{brandOf(product)}</span><button className="absolute left-4 top-4 z-10 grid h-9 w-9 place-items-center rounded-full bg-white text-slate-500 shadow-sm hover:text-red-400"><Heart className="h-4 w-4" /></button>{product.images[0]?.url ? <img className="product-image" src={product.images[0].url} alt={product.images[0].altText || product.title} /> : <Smartphone className="h-20 w-20 text-slate-300" />}</div><div className="p-5"><p className="text-[11px] font-bold text-slate-400">{product.description || "متاح بالتقسيط دون فوائد"}</p><h3 className="mt-1 text-lg font-extrabold text-[#071d32]">{product.title}</h3><div className="mt-5 flex items-end justify-between"><div><p className="font-mono text-2xl font-bold text-[#071d32]">${money(price)}</p><p className="mt-1 text-xs font-bold text-slate-500">{lira(price)} ل.س تقريباً</p></div><span className="rounded-lg bg-[#eaf8f4] px-2.5 py-1.5 text-[10px] font-extrabold text-[#17845e]">من ${money(Math.max(0, (price - 100) / 48))}/شهر</span></div><div className="mt-5 grid grid-cols-[1fr_46px] gap-2"><button onClick={details} className="button-dark h-11 rounded-xl text-sm">التفاصيل والتقسيط</button><button onClick={add} className="grid h-11 place-items-center rounded-xl border border-[#cfe0e2] text-[#25887e] hover:bg-[#eaf8f4]"><ShoppingBag className="h-4 w-4" /></button></div></div></article>; }
