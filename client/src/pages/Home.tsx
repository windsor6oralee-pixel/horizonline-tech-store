...
export default function Home() {
  ...
  return <div className="min-h-screen bg-[#f6f9fc] pb-[86px] md:pb-0" dir="rtl">
    ...
    <main id="top">
      ...
      <section id="store" className="bg-[#0a2342] py-12 sm:py-16"><div className="mx-auto max-w-7xl px-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[11px] font-extrabold tracking-[.15em] text-[#8ad9e3]">OUR STORE / 03</p><h2 className="mt-2 text-2xl font-extrabold tracking-tight text-white sm:text-3xl">متجرنا في حمص، على أرض الواقع.</h2><p className="mt-2 max-w-xl text-sm leading-7 text-[#c9d7e1]">زورونا لتجربة الأجهزة بنفسك، أو اطلب أونلاين ونوصّل مجاناً إلى باب بيتك في أي محافظة.</p></div><div className="hidden items-center gap-2 text-xs font-bold text-[#c6ecf3] sm:flex"><MapPin className="h-4 w-4" />الغوطة، مدينة حمص</div></div>
        <div className="mt-7 grid gap-4 lg:grid-cols-[1.5fr_1fr]">
          <figure className="group relative overflow-hidden rounded-[28px] border border-white/10"><img src="/images/store/storefront-wide.webp" alt="واجهة متجر Horizonline Tech Store في حمص" loading="lazy" className="h-[260px] w-full object-cover transition duration-700 group-hover:scale-[1.03] sm:h-[340px]" /><figcaption className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-[#061b37] via-[#061b37]/70 to-transparent p-5 pt-20"><p className="text-lg font-extrabold text-white">Horizonline Tech Store</p><p className="mt-1 text-xs font-bold text-[#c6ecf3]">الشارع الرئيسي، دخلة الرينبو، مقابل مطعم الخال، الغوطة</p></figcaption></figure>
          <div className="grid gap-4"><figure className="overflow-hidden rounded-[28px] border border-white/10"><img src="/images/store/storefront-square.webp" alt="داخل المتجر: أجهزة وإكسسوارات أصلية" loading="lazy" className="h-[200px] w-full object-cover object-top" /></figure><div className="grid grid-cols-3 gap-3"><StorePoint icon={<Smartphone className="h-4 w-4" />} text="صيانة جوالات" /><StorePoint icon={<Gift className="h-4 w-4" />} text="إكسسوارات" /><StorePoint icon={<Zap className="h-4 w-4" />} text="شواحن أصلية" /></div></div>
        </div></div></section>
...
function Journey({ no, title, text }: { no: string; title: string; text: string }) { return <div className="flex items-center gap-3 rounded-2xl bg-white/[.06] p-3"><span className="font-mono text-xs text-[#8ad9e3]">{no}</span><span><b className="block text-sm text-white">{title}</b><small className="mt-0.5 block text-[10px] text-[#c9d7e1]">{text}</small></span><ChevronLeft className="mr-auto h-4 w-4 text-[#8ad9e3]" /></div>; }
...
function StorePoint({ icon, text }: { icon: ReactNode; text: string }) { return <div className="flex flex-col items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/[.06] px-2 py-4 text-center text-[11px] font-extrabold text-white"><span className="grid h-9 w-9 place-items-center rounded-xl bg-[#8ad9e3] text-[#0a2342]">{icon}</span>{text}</div>; }
