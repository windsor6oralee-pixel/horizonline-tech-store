...
export default function Home() {
  ...
  return <div className="min-h-screen bg-[#f6f9fc] pb-[86px] md:pb-0" dir="rtl">
    ...
    <main id="top">
      ...
      <section id="delivery" className="scroll-mt-20 bg-white py-16 sm:py-20"><div className="mx-auto max-w-7xl px-5"><div className="grid gap-8 rounded-[32px] bg-[#f8fbfb] p-6 sm:p-10 lg:grid-cols-[.9fr_1.1fr] lg:items-center"><div><div className="dhl-card inline-flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-extrabold"><Truck className="h-5 w-5" />DHL Express</div><h2 className="mt-5 text-3xl font-extrabold tracking-tight text-[#0a2342] sm:text-4xl">شحن سريع بلا مقابل.</h2><p className="mt-4 max-w-md text-sm leading-8 text-slate-500">نرتّب توصيل هاتفك مجاناً إلى جميع المحافظات والمناطق السورية، مع متابعة للطلب حتى الاستلام.</p><div className="mt-6 rounded-2xl border border-slate-200 bg-white p-4"><p className="flex items-center gap-2 text-sm font-extrabold text-[#0a2342]"><Info className="h-4 w-4 text-[#1f6f96]" />استلام موثّق</p><p className="mt-2 text-xs leading-6 text-slate-500">يُرجى إبراز الهوية الشخصية للمندوب عند الاستلام، ثم مراجعة العقد وتوقيعه. <b className="text-[#a5301f]">لا يُسلَّم أي مبلغ نقدي للمندوب</b> — يُسدَّد الطلب عبر شام كاش قبل الشحن.</p></div></div><div className="grid gap-3 sm:grid-cols-2"><DeliveryPoint title="كل المحافظات" text="اختيار المحافظة والمنطقة في نموذج التوصيل" /><DeliveryPoint title="معلم قريب" text="وصف يساعد مندوب DHL على الوصول" /><DeliveryPoint title="رقم بديل اختياري" text="لضمان استلام سلس عند تعذر التواصل" /><DeliveryPoint title="توصيل مجاني" text="لا رسوم شحن إضافية على طلبك" /></div></div></div></section>
