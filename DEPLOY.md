...
### 5b. رقم واتساب المتجر
يُعدَّل من لوحة الإدارة `/admin` → بطاقة «رقم واتساب المتجر» (بالصيغة الدولية مع رمز الدولة).
يُحفظ في جدول `storeSettings`، لذلك يجب تنفيذ `pnpm db:push` مرة واحدة قبل أول حفظ؛ قبل ذلك يستخدم الموقع الرقم الافتراضي المضمّن في `shared/whatsapp.ts`.

### 5b2. التحقق الآلي من إيصالات الدفعة الأولى
أضف في Railway → خدمة الموقع → Variables: `ANTHROPIC_API_KEY` (مفتاح من console.anthropic.com).
بوجوده يقرأ النظام كل إيصال مرفوع ويطابق اسم المستلم مع «اسم صاحب المحفظة» في اللوحة، وتاريخ التحويل مع تاريخ الطلب، والمبلغ، ورقم العملية (منع التكرار) — ويفتح المحادثة تلقائياً عند النجاح. بدونه تبقى الإيصالات «قيد المراجعة» حتى تؤكدها يدوياً.

### 5c. ملفات العملاء (الهوية وإثبات الدفع)
تُحفظ داخل MySQL في جدول `storedFiles` ولا تحتاج أي خدمة تخزين خارجية. تُفتح من لوحة الإدارة فقط عبر `/api/files/<key>` (تتطلب جلسة مدير؛ أي طلب آخر يُرفض بـ 403).

### 5d. تشخيص قاعدة البيانات
من جلسة مدير: `GET /api/trpc/settings.dbStatus` يعرض مضيف القاعدة، حالة الترحيلات وسبب فشلها إن وُجد، وقائمة الجداول.
